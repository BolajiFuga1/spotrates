...
const BDC_CRIME_CONTEXT = ['terrorism', 'terror financ', 'terror funding', 'terrorist', 'sanctions on', 'sanction on']

const CBN_NON_FX_CONTEXT = [
  'parked',
  'trillion with the cbn',
  'liquidity mop',
  'open market operation',
  'om o auction',
]
...
export function isRelevantFxNews(item, strict = false) {
  ...
  const hasStrongFx = containsAny(blob, STRONG_FX_KEYWORDS)
  const hasNaira = containsAny(blob, ['naira', 'ngn'])
  ...
  if (hasBdc && containsAny(blob, BDC_CRIME_CONTEXT) && !hasStrongFx) return false

  if (hasCbn && containsAny(blob, CBN_NON_FX_CONTEXT) && !hasStrongFx && !hasBdc && !hasRemittance) {
    return false
  }

  if (hasCbn && !hasStrongFx && !hasBdc && !hasRemittance && !hasMonetary) return false

  if (hasNaira && !hasStrongFx && !hasCbn && !hasBdc && !hasMonetary && !hasRemittance) return false

  const relevant = hasStrongFx || hasCbn || hasBdc || hasMonetary || hasRemittance || hasNaira
  ...
  if (strict) {
    ...
    const nigeriaContext =
      ...
      n.includes(' lagos ') ||
      n.includes(' abuja ')
    if (!nigeriaContext && !hasCbn && !hasBdc) return false
  }
